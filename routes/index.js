...
router.get('/home/:rol', (req, res) => {
  respuesta = Administrador.VerPublicaciones.then((respuesta)=>{
...
if (req.body.rol == 'admin') {
    respuesta = Administradores.buscarAdm(req.body.Email).then(([userData])=>{
    ...
    respuesta = Administradores.buscarUser(req.body.Email).then(([userData])=>{
      bcrypt.compare(req.body.Password, userData.dataValues.Password, function(err, resp) {
        if (respuesta != false) {
          res.status(200).render('index', rol: req.body.rol});
        } else {
...
router.post('/register/:rol', validarUser, (req, res) => {
  ...
  if (req.params.rol === 'admin') {
    ...
    respuesta = Administradores.agregarAdmin(req.body).then(([user, created])=>{
      if (respuesta!=false) {
